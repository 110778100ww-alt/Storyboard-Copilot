...
import {
  ...
  DEFAULT_STYLE_VALUE,
  getStylePrompt,
} from '@/features/canvas/models/styles';
...
export const StoryboardGenNode = memo(({ id, data, selected, width, height }: StoryboardGenNodeProps) => {
  ...
  // 监听来自文本注释节点的连接事件
  useEffect(() => {
    return canvasEventBus.subscribe('text-annotation/fill-storyboard', ({ nodeId: sourceNodeId, frames }) => {
      // 检查是否连接到当前节点
      const isConnected = edges.some(
        (edge) => edge.source === sourceNodeId && edge.target === id
      );
      ...
      // 更新分镜描述，只更新描述字段，不覆盖现有帧
      const updatedFrames = [...nodeData.frames];
      
      frames.forEach((frame, index) => {
        if (index < updatedFrames.length) {
          // 更新现有帧的描述
          updatedFrames[index] = {
            ...updatedFrames[index],
  ...
  }, [id, edges, nodeData.frames, updateNodeData]);
  ...
  const buildPrompt = useCallback((): string => {
    ...
    const { gridRows, gridCols, frames, style } = nodeData;
    const parts: string[] = [];
    ...
    // 添加风格描述
    const stylePrompt = getStylePrompt(style ?? DEFAULT_STYLE_VALUE);
    if (stylePrompt) {
      promptDirectives.push(stylePrompt);
    }
  ...
  // 解析脚本文本，填充分镜描述
  const handleParseScriptText = useCallback((scriptText: string) => {
    if (!scriptText.trim()) return;

    const lines = scriptText.split('\n');
    const totalFrames = nodeData.gridRows * nodeData.gridCols;
    const newFrames: StoryboardGenFrameItem[] = [];
    
    // 初始化帧数组
    for (let i = 0; i < totalFrames; i++) {
      if (i < nodeData.frames.length) {
        newFrames.push({ ...nodeData.frames[i] });
      } else {
        newFrames.push({
          id: `frame-${Date.now()}-${i}`,
          description: '',
    ...
    for (const line of lines) {
      ...
      if (match) {
        const frameNumber = parseInt(match[1]) - 1; // 转换为0索引
        const content = match[2].trim();
        
        // 确保分镜序号有效
        if (frameNumber >= 0 && frameNumber < newFrames.length) {
          // 更新对应分镜的描述
      ...
      } else if (trimmedLine && currentFrameIndex >= 0 && currentFrameIndex < newFrames.length) {
        // 如果是当前分镜的后续内容，添加到描述中
        const currentDescription = newFrames[currentFrameIndex].description;
    ...
    // 同时更新frameDescriptionDrafts以确保UI正确显示
    const newDrafts: Record<string, string> = {};
    newFrames.forEach((frame) => {
      newDrafts[frame.id] = frame.description;
    });
    setFrameDescriptionDrafts(newDrafts);
  }, [id, nodeData.frames, nodeData.gridRows, nodeData.gridCols, updateNodeData]);
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        className={`mb-2 flex w-full items-center justify-center gap-1 rounded-md border border-[rgba(255,255,255,0.12)] bg-[rgba(255,255,255,0.04)] px-2 py-1 text-xs transition-colors ${
          showScriptInput
        ...
        onClick={(event) => {
          event.stopPropagation();
          setShowScriptInput(!showScriptInput);
        }}
      >
        <FileText className="h-3 w-3" />
        <span>{t('node.storyboardGen.scriptInput', '脚本文本')}</span>
      </button>
      ...
      {showScriptInput && (
        <div className="mb-2 flex flex-col gap-2">
          <textarea
            value={nodeData.scriptText ?? ''}
            onChange={(event) => {
              const nextValue = event.target.value;
              updateNodeData(id, { scriptText: nextValue });
            }}
            placeholder={t('node.storyboardGen.scriptPlaceholder', '输入脚本文本，如：分镜1：主角站在海边\n分镜2：主角转身看向远方')}
            className="nodrag nowheel h-20 w-full resize-none rounded-md border border-[rgba(255,255,255,0.12)] bg-[rgba(255,255,255,0.04)] px-2 py-1 text-xs text-text-dark outline-none placeholder:text-text-muted/40 focus:border-accent/50"
          />
          <button
            type="button"
            onClick={(event) => {
              event.stopPropagation();
              if (nodeData.scriptText?.trim()) {
                handleParseScriptText(nodeData.scriptText);
              }
            }}
            disabled={!nodeData.scriptText?.trim()}
            className="flex items-center justify-center gap-1 rounded-md border border-[rgba(255,255,255,0.12)] bg-accent/80 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-accent disabled:cursor-not-allowed disabled:opacity-50"
          >
            <Sparkles className="h-3 w-3" />
            <span>{t('node.storyboardGen.parseAndFill', '识别并填充')}</span>
          </button>
        </div>
      )}
      ...
      >
        <ModelParamsControls
          ...
          selectedStyle={nodeData.style ?? DEFAULT_STYLE_VALUE}
          styleOptions={IMAGE_STYLE_OPTIONS.map((option) => ({
            value: option.value,
            label: option.label,
          }))}
          onStyleChange={(style) => updateNodeData(id, { style })}
      ...
      <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-blue-400/50 border-2 border-blue-500 pointer-events-none z-10" />
      <Handle
        type="target"
        id="target"
        position={Position.Left}
        className="!h-6 !w-6 !-ml-3 !bg-transparent !border-0"
      />
