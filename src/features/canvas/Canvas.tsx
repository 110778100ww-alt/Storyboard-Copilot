...
export function Canvas() {
  ...
  const spacePanGestureRef = useRef<{
    active: boolean;
    pointerId: number;
    startClientX: number;
    startClientY: number;
  ...
  useEffect(() => {
    ...
    const handleKeyUp = (event: KeyboardEvent) => {
      if (event.code === 'Space') {
        ...
        spacePanGestureRef.current = null;
      }
    };
  ...
  useEffect(() => {
    if (selectedNodeIds.length >= 2) {
      ...
      const groupNode = selectedNodes.find(n => n.type === CANVAS_NODE_TYPES.group);
      if (groupNode) {
        return;
      }
      ...
      if (selectedNode && selectedNode.type === CANVAS_NODE_TYPES.group) {
  ...
  }, [selectedNodeIds, nodes, groupNodes, ungroupNode, scheduleCanvasPersist]);
  ...
  }, [addNode, reactFlowInstance, updateNodeData]);
  ...
  return (
    <div ref={wrapperRef} className="relative h-full w-full">
      <ReactFlow
        ...
        className={`bg-bg-dark ${spacePressed ? 'cursor-grab' : ''}`}
        onPointerDown={(event) => {
        ...
        onPointerMove={(event) => {
          const gesture = spacePanGestureRef.current;
          if (!gesture || !gesture.active || event.pointerId !== gesture.pointerId) {
            return;
          }
          const deltaX = event.clientX - gesture.startClientX;
      ...
      {isDragOverCanvas && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-blue-500/20 border-2 border-dashed border-blue-500 pointer-events-none">
          <div className="flex flex-col items-center gap-2 p-4 rounded-lg bg-white/90 shadow-lg">
            <Upload className="w-12 h-12 text-blue-600" />
            <span className="text-sm font-medium text-gray-900">松开导入图片</span>
          </div>
        </div>
      )}
