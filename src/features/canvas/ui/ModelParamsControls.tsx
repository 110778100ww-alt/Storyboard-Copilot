...
export const ModelParamsControls = memo(({
  ...
  const hasOtherParamsPanel = showWebSearchToggle || inlineExtraParamSchema.length > 0 || (styleOptions && styleOptions.length > 0);
          ...
          <UiPanel className={OTHER_PARAMS_PANEL_CLASS_NAME}>
            <div className="space-y-3">
              ...
              {styleOptions && styleOptions.length > 0 && (
                <div className="space-y-2 rounded-lg border border-[rgba(255,255,255,0.08)] bg-bg-dark/65 p-3">
                  <div>
                    <div className="text-xs font-medium text-text-dark">
                      {t('modelParams.style', '图像风格')}
                    </div>
                    <div className="mt-0.5 text-[11px] leading-4 text-text-muted">
                      {t('modelParams.styleDesc', '选择生成图像的风格')}
                    </div>
                  </div>
                  <UiSelect
                    value={selectedStyle ?? DEFAULT_STYLE_VALUE}
