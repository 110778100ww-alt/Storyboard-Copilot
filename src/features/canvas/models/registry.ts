...
const textModels: TextModelDefinition[] = Object.values(textModelModules)
...
const textModelMap = new Map<string, TextModelDefinition>(
  textModels.map((model) => [model.id, model])
);

export const DEFAULT_IMAGE_MODEL_ID = 'grsai/nano-banana-pro';
export const DEFAULT_TEXT_MODEL_ID = 'google/gemini-2.0-flash';
...
export function getTextModel(modelId: string): TextModelDefinition {
  return textModelMap.get(modelId) ?? textModelMap.get(DEFAULT_TEXT_MODEL_ID)!;
}
