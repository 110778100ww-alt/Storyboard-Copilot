...
export function SettingsDialog({
  ...
  const providers = useMemo(() => {
    const providerOrder = ['kie', 'ppio', 'fal', 'grsai', 'google'];
  ...
  const handleSave = useCallback(async () => {
    ...
    // 为Google AI提供商设置文本模型API密钥（始终调用，确保密钥被设置）
    try {
      const { setTextApiKey } = await import('@/commands/text');
      const googleApiKey = localApiKeys['google'] ?? '';
      await setTextApiKey('google', googleApiKey);
      console.info('[Settings] Google AI API key set:', googleApiKey ? '***' + googleApiKey.slice(-4) : '(empty)');
    } catch (error) {
      console.error('[Settings] Failed to set Google AI API key:', error);
    }
