...
use crate::ai::providers::{build_default_providers, build_default_text_providers};
use crate::ai::{
    GenerateRequest, ProviderRegistry, ProviderTaskHandle, ProviderTaskPollResult,
    ProviderTaskSubmission, TextGenerationRequest,
};
...
fn get_registry() -> &'static ProviderRegistry {
    REGISTRY.get_or_init(|| {
        ...
        for provider in build_default_text_providers() {
            registry.register_text_provider(provider);
        }
        registry
    })
}
...
pub struct TextGenerationResponseDto {
    pub text: String,
    pub model: String,
    pub usage: Option<TextUsageDto>,
}

#[derive(Debug, Serialize, Deserialize)]
pub struct TextUsageDto {
...
pub async fn generate_text(request: TextGenerationRequestDto) -> Result<TextGenerationResponseDto, String> {
    ...
    let registry = get_registry();
    let provider = registry
        .resolve_text_provider_for_model(&request.model)
        .or_else(|| registry.get_default_text_provider())
        .ok_or_else(|| "Text model provider not found".to_string())?;
    ...
    let response = provider.generate_text(req).await.map_err(|e| e.to_string())?;
    
    Ok(TextGenerationResponseDto {
        text: response.text,
        model: response.model,
        usage: response.usage.map(|u| TextUsageDto {
...
#[tauri::command]
pub async fn list_text_models() -> Result<Vec<String>, String> {
    Ok(get_registry().list_text_models())
}

#[tauri::command]
pub async fn set_text_api_key(provider: String, api_key: String) -> Result<(), String> {
    info!("Setting text API key for provider: {}", provider);

    let registry = get_registry();
    let resolved_provider = registry
        .get_text_provider(provider.as_str())
        .ok_or_else(|| format!("Unknown text provider: {}", provider))?;
